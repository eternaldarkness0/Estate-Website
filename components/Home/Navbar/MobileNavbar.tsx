...
const MobileNavbar = ({closeNav, activeNav} : Props) => {
  ...
  return (
    ...
    <div>
      <div className={`${navOpen} fixed transform transition-all duration-500 inset-0 z-[1000] bg-black opacity-70 w-full`}>
      </div>

      <div className={`${navOpen} text-white transform transition-all duration-500 delay-300 fixed flex justify-center flex-col h-full w-[80%] sm:w-[60%] bg-[#c1205e] space-y-6 z-[1001]`}>
        {navLinks.map((item) => {
            return (
